...
"""
istac_poblacion.py
Descarga el dataset C00025A_000002 (Población de derecho, Canarias,
islas y municipios) del ISTAC y lo guarda como CSV en
./tmp/poblacion_YYYYMMDD.csv

El cubo tiene 3 dimensiones:
  MEDIDAS      → POBLACION | POBLACION_VA | POBLACION_TVA
  TIME_PERIOD  → años 1986-2024
  TERRITORIO   → Canarias, islas, municipios (97 territorios)

Solo se extrae MEDIDAS = POBLACION (población de derecho).

Sin dependencias externas.

Uso:
    python3 istac_poblacion.py
    python3 istac_poblacion.py --raw    # guarda también el JSON crudo
"""
...
DATASET_URL = (
    ...
    "/v1.0/datasets/ISTAC/C00025A_000002/~latest?_type=json"
)
...
def build_csv(raw: dict) -> list[dict]:
    """
    Reconstruye las observaciones filtrando solo POBLACION,
    y devuelve lista de dicts con columnas:
        territorio_codigo, territorio_nombre, periodo, poblacion

    El orden de dimensiones en este dataset es:
        TERRITORIO × MEDIDAS × TIME_PERIOD
    (comprobado inspeccionando raw['data']['dimensions']['dimension'])
    """
    ...
    print(f"  Medidas disponibles: {medidas}")

    rows = []
    idx  = 0
    # Orden de iteración según posición en data['dimensions']: TERRITORIO × MEDIDAS × TIME_PERIOD
    for terr_code in territorios:
        for medida in medidas:
            for periodo in periodos:
                ...
                # Solo nos interesa POBLACION (población de derecho)
                if medida != "POBLACION":
                    continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Descarga población de derecho del ISTAC (C00025A_000002)"
    )
    parser.add_argument(
    ...
    csv_path = TMP_DIR / f"poblacion_{date_str}.csv"

    if csv_path.exists():
        print(f"Ya existe: {csv_path} — omitiendo.")
        sys.exit(0)

    print(f"Descargando dataset ISTAC C00025A_000002...")
    ...
    if args.raw:
        raw_path = TMP_DIR / f"poblacion_raw_{date_str}.json"
    ...
    print("\nPrimeras 6 filas:")
    print(f"  {'CODIGO':<12} {'NOMBRE':<40} {'AÑO':<6} {'POBLACION'}")
    print(f"  {'-'*12} {'-'*40} {'-'*6} {'-'*12}")
    for row in rows[:6]:
        val = f"{row['poblacion']:,.0f}" if row['poblacion'] is not None else "NULL"
