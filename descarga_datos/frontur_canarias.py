...
"""
frontur_canarias.py
Descarga el dataset E16028B_000016 (FRONTUR — Estadística de Movimientos
Turísticos en Fronteras) del ISTAC y guarda el total de turistas llegados
por territorio y mes en ./tmp/frontur_YYYYMMDD.csv

Territorios: ES70 (Canarias total), ES704 (Fuerteventura), ES705 (Gran Canaria),
  ES707 (La Palma), ES708 (Lanzarote), ES709 (Tenerife).
  El Hierro (ES703) y La Gomera (ES706) no están en el dataset.

Se extraen solo períodos mensuales (YYYY-Mxx), TIPO_VIAJERO=TURISTA_PRINCIPAL,
LUGAR_RESIDENCIA=_T, MEDIDAS=TURISTAS.

Sin dependencias externas.

Uso:
    python3 frontur_canarias.py
    python3 frontur_canarias.py --raw
"""
...
DATASET_URL = (
    ...
    "/v1.0/datasets/ISTAC/E16028B_000016/~latest?_type=json"
)
...
def build_csv(raw: dict) -> list[dict]:
    ...
    Extrae solo períodos mensuales (YYYY-Mxx), TIPO_VIAJERO=TURISTA_PRINCIPAL,
    LUGAR_RESIDENCIA=_T, MEDIDAS=TURISTAS.
    """
    ...
    tv_idx = tipos.index("TURISTA_PRINCIPAL")
    lr_idx = residencias.index("_T")
    m_idx  = medidas.index("TURISTAS")

    rows = []
    for p_i, periodo in enumerate(periodos):
        # Solo períodos mensuales (formato YYYY-Mxx); los anuales (YYYY) tienen len=4
        if "-M" not in periodo:
            continue
        year = int(periodo[:4])
        mes  = int(periodo[6:8])
...
def main():
    parser = argparse.ArgumentParser(
        description="Descarga FRONTUR por territorio del ISTAC (E16028B_000016)"
    )
    ...
    csv_path = TMP_DIR / f"frontur_{date_str}.csv"

    if csv_path.exists():
        print(f"Ya existe: {csv_path} — omitiendo.")
        sys.exit(0)

    print("Descargando dataset ISTAC E16028B_000016 (FRONTUR)...")
    ...
    if args.raw:
        raw_path = TMP_DIR / f"frontur_raw_{date_str}.json"
    ...
    for r in sorted(muestra, key=lambda r: r["territorio_codigo"]):
